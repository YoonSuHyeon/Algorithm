...
public class 택배상자 {
    public static int solution(int[] order) {
        ...
        while (true) {
            ...
            if (orders.peek() >= maxOrder)
                IntStream.rangeClosed(maxOrder + 1, orders.peek()).forEach(tempStack::push);

            Integer peek = tempStack.pop();
            Integer peek1 = orders.poll();

            //실을 수 없는 경우
            if (!peek.equals(peek1))
                break;

            maxOrder = Math.max(maxOrder, peek1);
            result++;
        }
    ...
    public static void main(String[] args) {
        int[] order = {2, 3, 1, 4, 5};
        System.out.println("result = " + 택배상자.solution(order));
    }
