...
public class 롤케이크자르기 {
    ...
    public static int solution(int[] toppings) {
        ...
        Map<Integer, Integer> map = Arrays.stream(toppings).boxed().collect(Collectors.groupingBy(identity(), summingInt(x -> 1)));
        Set<Integer> brotherToppings = new HashSet<>();
        int answer = 0;
        ...
        for (int topping : toppings) {
            ...
            //동생에게 토핑을 한개씩 준다.
            //내꺼에서 한개를 제거한다.
            int toppingCnt = map.getOrDefault(topping, 0);
            if (toppingCnt <= 1)
                map.remove(topping);
            else
                map.replace(topping, toppingCnt - 1);

            brotherToppings.add(topping);

            if (map.keySet().size() == brotherToppings.size())
                answer++;
    ...
    public static void main(String[] args) {
        ...
        System.out.println("result = " + 롤케이크자르기.solution(topping));
    }
}
